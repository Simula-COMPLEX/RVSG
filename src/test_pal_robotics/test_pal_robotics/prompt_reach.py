...
- Most importantly, design the roles, workflows, and task sequences so that the natural activities of the two workers are likely to cause indirect disruption to the robot’s motion along its fixed route ({}), in a way that the scenario execution leads to the robot taking the longest possible time to reach its goal (i.e., the slowest possible execution along its fixed path).
- Explicitly describe how and why worker behaviors or process flows may lead to a longer total time for the robot to reach its goal, focusing on realistic environmental disruptions such as temporary obstructions, adjacent activity, or object placement that cause the robot to stop, wait, or take detours, thereby increasing its total travel time along its fixed route ({}).
...
prompt_reach_2 = """You are a warehouse operations process designer. Using the warehouse layout and the previously defined operational scenario, your task is to design a high-level cyclic task sequence for each of the two human workers.
...
prompt_reach_4 = """You are a warehouse route validation specialist. Your task is to carefully review all previously generated waypoint paths for each worker, according to the following requirements:
...
prompt_reach_5 = """You are a warehouse worker route and task reporter. Based on the defined task and navigation sequences for both workers, your job is to output the entire process in the following strict JSON structure:
...
The current scenario produces a time_to_reach_goal value of **{}**. Your target is to design operational changes that will increase this value as much as possible, while keeping the overall scenario logic and operational goals unchanged.
...
prompt_reach_memory_1 = """You are a warehouse operations scenario planner. Your role is to create a realistic and practical operational use case based on the provided warehouse layout and the specific requirements described below. Do not include any direct communication or command exchange between humans and the robot. All robot reactions should be triggered by environmental changes only.
